...
init python:
    ...
    build.classify("README.html", None) #Included help file for mod installation
    build.classify("game/python-packages/**", None)#Additional python pacakges
    build.classify("CustomIcon**.**", None)

    build.classify('README_jp.html', build.name)
    build.classify('readme (mas jp patch).txt', build.name)
